...
# ==============================================================================
    ...
    if st.session_state.user_role != "VFHTP":
        ...
        st.markdown('<div class="sub-title">⚙️ QUẢN LÝ & KHÓA TÀI KHOẢN NHÂN VIÊN</div>', unsafe_allow_html=True)
        
        nv_target = st.text_input("Nhập Mã NV cần xử lý: *", placeholder="Ví dụ: NV001")
        
        action = st.radio("Thao tác muốn thực hiện:", [
            "🔒 Khóa tài khoản (Nhân viên nghỉ việc)",
            "🔓 Mở khóa / Duyệt tài khoản",
            "🔑 Đổi mật khẩu nhân viên",
            "📱 Reset thiết bị đăng nhập"
        ])
        
        pass_moi = ""
        if action == "🔑 Đổi mật khẩu nhân viên":
            pass_moi = st.text_input("Mật khẩu mới cho NV: *", type="password")
            
        if st.button("⚡ THỰC THI THAY ĐỔI", type="primary", use_container_width=True):
            if not nv_target.strip():
                st.error("⚠️ Vui lòng nhập Mã NV!")
            else:
                if "Khóa tài khoản" in action:
                    ok, msg = cap_nhat_nhan_vien(nv_target, trang_thai_moi="Đã nghỉ/Khóa")
                elif "Mở khóa" in action:
                    ok, msg = cap_nhat_nhan_vien(nv_target, trang_thai_moi="Đã duyệt")
                elif "Đổi mật khẩu" in action:
                    if not pass_moi.strip():
                        st.error("⚠️ Vui lòng nhập mật khẩu mới!")
                        ok, msg = False, ""
                    else:
                        ok, msg = cap_nhat_nhan_vien(nv_target, mat_khau_moi=pass_moi)
                elif "Reset thiết bị" in action:
                    ok, msg = cap_nhat_nhan_vien(nv_target, reset_device=True)
